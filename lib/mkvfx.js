...
function execTask(task, workingDir) {
    ...
    try {
        ...
        if (e.message != "spawnSync ENOTCONN") {
            ...
            stdout.write(e.name);
            stdout.write(e.fileName);
            stdout.write(e.lineNumber);
            stdout.write(e.columnNumber);
...
function validate_tool_chain() {
    ...
    if (!fs.existsSync(mkvfx_root)) {
        fs.mkdir(mkvfx_root, function(err) {
            if (err) {
                cursor.red();
                stdout.write("MKVFX Could not create dir: %s", mkvfx_root);
                cursor.fg.reset();
                throw err;
            }
        });
    ...
    if (!fs.existsSync(mkvfx_root+"/bin")) {
        fs.mkdir(mkvfx_root+"/bin", function(err) {
            if (err) {
                cursor.red();
                stdout.write("MKVFX Could not create dir: %s", mkvfx_root+"/bin");
                cursor.fg.reset();
                throw err;
            }
        });
    ...
    if (!fs.existsSync(mkvfx_root+"/include")) {
        fs.mkdir(mkvfx_root+"/include", function(err) {
            if (err) {
                cursor.red();
                stdout.write("MKVFX Could not create dir: %s", mkvfx_root+"/include");
                cursor.fg.reset();
                throw err;
            }
        });
    ...
    if (!fs.existsSync(mkvfx_root+"/lib")) {
        fs.mkdir(mkvfx_root+"/lib", function(err) {
            if (err) {
                cursor.red();
                stdout.write("MKVFX Could not create dir: %s", mkvfx_root+"/lib");
                cursor.fg.reset();
                throw err;
            }
        });
    ...
    if (!fs.existsSync(mkvfx_root + "/man")) {
        fs.mkdir(mkvfx_root + "/man", function(err) {
            if (err) {
                cursor.red();
                stdout.write("MKVFX Could not create dir: %s", mkvfx_root + "/man");
                cursor.fg.reset();
                throw err;
            }
        });
    ...
    if (!fs.existsSync(mkvfx_root + "/man/man1")) {
        fs.mkdir(mkvfx_root + "/man/man1", function(err) {
            if (err) {
                cursor.red();
                stdout.write("MKVFX Could not create dir: %s", mkvfx_root + "/man/man1");
                cursor.fg.reset();
                throw err;
            }
        });
    ...
    if (!fs.existsSync(mkvfx_source_root)) {
        fs.mkdir(mkvfx_source_root, function(err) {
            if (err) {
                cursor.red();
                stdout.write("MKVFX Could not create dir: %s", mkvfx_source_root);
                cursor.fg.reset();
                throw err;
            }
        });
    ...
    if (!fs.existsSync(mkvfx_build_root)) {
        fs.mkdir(mkvfx_build_root, function(err) {
            if (err) {
                cursor.red();
                stdout.write("MKVFX Could not create dir: %s", mkvfx_build_root);
                cursor.fg.reset();
                throw err;
            }
        });
...
function runRecipe(recipe, package_name, p, dir_name, execute) {
	stdout.write("package:%s recipe: %s", package_name, recipe);

	var build_dir = mkvfx_root;
	if ("build_in" in p) {
		build_dir = substitute_variables(p.build_in);
		stdout.write("in directory %s", build_dir);
	}
	else {
	...
	for (r = 0; r < recipe.length; ++r) {
		if (execute) {
			...
			stdout.write("Simulating: %s", substitute_variables(recipe[r]));
        }
	}
...
function bake(package_name) {
	stdout.write("Baking %s", package_name);
	...
	for (var i = 0; i < package_recipes.length; ++i) {
		if (package_recipes[i].name === package_name) {
			...
			if (option_do_dependencies) {
				...
				stdout.write("Dependencies of %s baked, moving on the entree", package_name);
			}
			...
			if ("repository" in p) {
				stdout.write("Fetching %s", package_name);
				...
				if (option_do_fetch) {
					...
					if ("type" in repository && url !== "") {
						...
						if (type == "git") {
							...
							stdout.write("curl from: %s", dir_path);
							if (!fs.existsSync(dir_path)) {
								fs.mkdir(dir_path, function(err) {
									if (err) {
                    cursor.red();
                    stdout.write("MKVFX Could not find create dir: %s", dir_path);
                    cursor.fg.reset();
                    throw err;
									}
								});
				...
				stdout.write("Repository not specified for %s, not fetching", package_name);
			}

			if (option_do_build) {
				cursor.yellow();
				stdout.write("Building %s recipe", package_name);
				cursor.fg.reset();
			...
			if (option_do_install) {
				stdout.write("Installing %s", package_name);
...
function printHelp() {
    ...
    cursor.yellow();
    for (var i = 0; i < package_recipes.length; ++i) {
        stdout.write("%s", package_recipes[i].name);
    }
    cursor.fg.reset();
...
fs.readFile(__dirname + '/recipes.json', 'utf8', function(err, data) {
    ...
    for (arg = 0; arg < args.length; ++arg) {
        ...
        if (argLower == "--help") {
            ...
            cursor.red();
            stdout.write("Unknown option %s", args[arg]);
            cursor.fg.reset();
