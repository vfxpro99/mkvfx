...
function validate_tool_chain() {
    ...
    if (!fs.existsSync(mkvfx_root)) {
        fs.mkdir(mkvfx_root, function(err) {
            if (err) {
                cursor.red();
                stdout.write("MKVFX Could not create dir: %s\n", mkvfx_root);
                cursor.fg.reset();
                throw err;
            }
        });
...
function runRecipe(recipe, package_name, p, dir_name, execute) {
	...
	if (!fs.existsSync(build_dir)) {
		try {
			...
			cursor.red();
			stdout.write("Couldn't create build directory %s\n", build_dir);
			cursor.fg.reset();
	...
	for (r = 0; r < recipe.length; ++r) {
		if (execute) {
			...
			stdout.write("Simulating: %s\n", substitute_variables(recipe[r]));
        }
	}
	...
	for (var i = 0; i < package_recipes.length; ++i) {
		if (package_recipes[i].name === package_name) {
			...
			if (option_do_dependencies) {
				...
				stdout.write("Dependencies of %s baked, moving on the entree\n", package_name);
			}
			...
			if ("repository" in p) {
				...
				if (option_do_fetch) {
					...
					if ("type" in repository && url !== "") {
						...
						if (type == "git") {
							...
							stdout.write("curl from: %s\n", dir_path);
							if (!fs.existsSync(dir_path)) {
								fs.mkdir(dir_path, function(err) {
									if (err) {
                    cursor.red();
                    stdout.write("MKVFX Could not find create dir: %s\n", dir_path);
                    cursor.fg.reset();
                    throw err;
									}
								});
				...
				stdout.write("Repository not specified for %s, not fetching.\n", package_name);
			}

			if (option_do_build) {
				cursor.yellow();
				stdout.write("Building %s recipe\n", package_name);
				cursor.fg.reset();
				if (platform_recipe in p) {
					...
					cursor.red();
					stdout.write("No recipe exists for %s\n", package_name);
					cursor.fg.reset();
			...
			if (option_do_install) {
				...
				if (platform_install in p) {
					...
					cursor.red();
					stdout.write("No install exists for %s\n", package_name);
					cursor.fg.reset();
