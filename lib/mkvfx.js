...
function check_for_7zip() {
    ...
    found7zip = !result; // because zero means success
	return found7zip;
}
...
function check_for_make() {
    ...
    foundMake = !result; // because 0 means success
	return foundMake;
}
...
function check_for_cmake() {
    ...
    foundCmake = !result; // because 0 is success
	return foundCmake;
}
...
function check_for_premake() {
	...
	var result = 1;
	try {
		result = execTask('premake4 --version');
	}
	catch (err) {
	}
...
var recipe_path = __dirname + '/' + recipes_file;
fs.readFile(recipe_path, 'utf8', function(err, data) {
    if (err) {
        cursor.red();
        stdout.write("MKVFX Could not read recipes: " + recipe_path);
        cursor.fg.reset();
        throw err;
    }
    ...
    if (!args.length && !option_build_all) {
        printHelp();
        var err = [];
        err.message = "Nothing to build";
    }
    else {
        if (option_build_all) {
            for (var i = 0; i < lower_case_map.length; ++i) {
                bake(lower_case_map[i]);
            }
        }
        else for (arg = 0; arg < to_build.length; ++arg) {
            var build_me = to_build[arg].toLowerCase();
            if (build_me in lower_case_map)
                bake(lower_case_map[build_me]);
        }
    }
